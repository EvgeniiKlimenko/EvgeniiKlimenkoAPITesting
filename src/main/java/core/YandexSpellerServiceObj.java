...
public class YandexSpellerServiceObj {
    ...
    public static List<YandexSpellerAnswer> getAnswers(Response response) {
        return new Gson()
                .fromJson(response.asString().trim(), new TypeToken<List<YandexSpellerAnswer>>() {}.getType());
    }

    public static List<String> getStringResult(Response response) {
        return getAnswers(response).stream()
                .map(yandexSpellerAnswer -> yandexSpellerAnswer.getWord()).collect(Collectors.toList());
    }
